...
the recorded result will be returned. This allows the test to verify correctness without connecting to the database. The tests also
executes at faster speed in this mode. By default, `dbtest.WithDBPlayback` will connect to a database using the following
connection parameters, which assumes a CockroachDB instance running on local host. 
