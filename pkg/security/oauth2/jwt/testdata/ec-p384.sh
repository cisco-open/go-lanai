...
do
  ...
  # new keys
  openssl ecparam -name secp384r1 -genkey -noout -out $TMP_DIR/ec-p384-priv-key-trad-$i.pem
  openssl pkcs8 -topk8 -nocrypt -in $TMP_DIR/ec-p384-priv-key-trad-$i.pem -out $TMP_DIR/ec-p384-priv-key-$i.pem
...
cat `find $TMP_DIR -type f -name 'ec-p384-priv-key-*.pem' -a ! -name '*ec-p384-priv-key-aes256-*.pem' -a ! -name '*ec-p384-priv-key-trad-*.pem' | sort` > ec-p384-priv-key.pem
cat `find $TMP_DIR -type f -name 'ec-p384-priv-key-trad-*.pem'` > ec-p384-priv-key-trad.pem
cat `find $TMP_DIR -type f -name 'ec-p384-priv-key-aes256-*.pem'` > ec-p384-priv-key-aes256.pem
cat `find $TMP_DIR -type f -name 'ec-p384-pub-key-*.pem' | sort` > ec-p384-pub-key.pem
cat `find $TMP_DIR -type f -name 'ec-p384-*.crt' | sort` > ec-p384-cert.pem
