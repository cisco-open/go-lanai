...
do
  ...
  # new keys
  openssl genpkey -algorithm ed25519 -out $TMP_DIR/ed25519-priv-key-trad-$i.pem
  openssl pkcs8 -topk8 -nocrypt -in $TMP_DIR/ed25519-priv-key-trad-$i.pem -out $TMP_DIR/ed25519-priv-key-$i.pem
...
cat `find $TMP_DIR -type f -name 'ed25519-priv-key-*.pem' -a ! -name '*ed25519-priv-key-aes256-*.pem' -a ! -name '*ed25519-priv-key-trad-*.pem' | sort` > ed25519-priv-key.pem
cat `find $TMP_DIR -type f -name 'ed25519-priv-key-trad-*.pem' | sort` > ed25519-priv-key-trad.pem
